...
public class TransitionalObject : MonoBehaviour
{
    ...
    public BaseTransition FirstTransition
    {
        get
        {
    ...
    /// <summary>
    /// A helper for automatically casting a MovingTransition
    /// </summary>
    public MovingTransition MovingTransition
    {
        get
        {
            if(transitions.Length > 0)
                return (MovingTransition)transitions[0];
            else
                return null;
        }
    }
    ...
    /// <summary>
    /// A helper for automatically casting a ScalingTransition
    /// </summary>
    public ScalingTransition ScalingTransition
    {
        get
        {
            if(transitions.Length > 0)
                return (ScalingTransition)transitions[0];
            else
                return null;
        }
    }
    ...
    #region Helpers
    /// <summary>
    /// Returns the percentage of the first transition
    /// </summary>
    /// <returns></returns>
    public float TransitionPercentage
    {
        ...
        {
            if(transitions.Length > 0)
                return transitions[0].TransitionPercentage;
            else
                return 0;
        }
    }

    public BaseTransition.TransitionState State
    {
        get
        {
            if(transitions.Length > 0)
                ...
                return BaseTransition.TransitionState.Finished;
        }

        set
        {
            for(int i = 0; i < transitions.Length; i++)
                transitions[i].state = value;
        }
    }
    #endregion
    ...
    /// <summary>
    /// This sets the state for Every base transition that is apart of this object
    /// </summary>
    public void SetState(BaseTransition.TransitionState state)
    {
        for(int i = 0; i < transitions.Length; i++)
    ...
    public static Transform CreateTransform(GameObject current, string label)
    {
        ...
        if(holder != null)
            ...
            temp.transform.SetParent(current.transform.parent);
